 ...
 * @LastEditTime: 2022-11-20 17:41:29
...
async function esbuildScanPlugin(config, depImports) {
    ...
    //1.总结下步骤，调用 resovlePlugin 后将插件一个数组放到了 config 的 plugin 上
    config.plugins = [resolvePlugin(config)]; //vite 插件
    ...
    const resolve = async (id, importer) => {
        //插件容器进行路径解析，返回绝对路径
        //2 调用 container.resolveId 的时候，遍历上面的 plugin 插件
    ...
    return {
        ...
        setup(build) {
            ...
            //处理.vue 类型的文件，如果遇到 vue 文件，返回它的绝对路径，并且标识为外部依赖，不在进一步解析
            build.onResolve({
                    filter: /\.vue$/
                },
                async ({
